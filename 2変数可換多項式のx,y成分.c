...
int main(int argc, char const *argv[])
{
	...
	for (m = 1; m <= 10; ++m)						//1,2,3,・・・,mとTm(x,y)のmを変化させていく
	{
		xx=Tm_x(m);
		yy=Tm_y(m);
		printf("T%d(x,y)=(%d,%d)\n",m,xx,yy);
		printf("x->\t");surplus(xx);				//xの剰余演算
		printf("y->\t");surplus(yy);				//yの剰余演算				
		printf("\n");
	}
	return 0;
}
...
void surplus(int huga){
	...
	remnant=huga%piyo;								//Tm(x,y)%2^k　　＊ ^ <- このキャレットは冪の意味
		...
		// 	printf("-----------------------\n");
		// }
	if(remnant<0){									//C言語の特性上無理やり負数の余りを+に
		remnant+=piyo;
	}
...
int Tm_x(int m){									//2変数可換多項式のxの式
	...
	return x*Tm_x(m-1)-y*Tm_x(m-2);					//定義式
}
